...
class Testing(unittest.TestCase):
  ...
  ########################################
  def llm_test1(self):
    rsp = requests.post("http://localhost:3000/llm", json = {"prompt" : "What is the melting point of silver?"})
    self.assertEqual(rsp.status_code, 200)
    print(rsp.json())
    self.assertTrue("961" in rsp.json()["output"])

    rsp = requests.post("http://localhost:3000/llm", json = {"string" : "Gigavolt Havoc"})
    self.assertEqual(rsp.status_code, 400)
    self.assertDictEqual(rsp.json(), {})

    rsp = requests.post("http://localhost:3000/llm", json = {"prompt" : 54})
    self.assertEqual(rsp.status_code, 400)
    self.assertDictEqual(rsp.json(), {})
